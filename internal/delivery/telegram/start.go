...
func (h *Handler) StartCommand(c tele.Context) error {
	...
	case chatPkg.StatusCreated:
		return c.Send("Я тут новенький, сохраняю чат в базу.\n" +
			"Отправьте /select_group для выбора группы. Список всех команд доступен в /help")
	case chatPkg.StatusFound:
		return c.Send("Зачем стартовать снова? Я уже и так тут есть. Для помощи введите /help")
	default:
